...
class SupConLoss(nn.Module):
    ...
    def forward(self, features, labels=None, mask=None, focuses=None, focus_labels=None):
        ...
        contrast_feature = torch.cat(torch.unbind(features, dim=1), dim=0)  # remove n_view, [2N, dims]
        # pdb.set_trace()
        if focus_labels is not None:
        ...
        anchor_dot_contrast = torch.div(
            ...
            self.temperature)   # I, all z_ij, 2N

        # for numerical stability
        logits_max, _ = torch.max(anchor_dot_contrast, dim=1, keepdim=True)
        logits = anchor_dot_contrast - logits_max.detach()  # avoid very large logits that lead to the NaN problem
        ...
        logits_mask[:, batch_size * anchor_count:] = logits_mask[:, batch_size * anchor_count:] * 10
